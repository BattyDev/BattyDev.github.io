...
-- seed
-- ---------------------------------------------------------------------------
-- Inserting into auth.users only -- public.members rows come from the
-- on_auth_user_created trigger, which is how a real Discord login provisions a
-- member. If the members rows appear below, that trigger works.
insert into auth.users (id, email, raw_user_meta_data) values
  ('11111111-1111-1111-1111-111111111111', 'batty@example.com',
   '{"provider_id":"100000000000000001","full_name":"Batty","avatar_url":"https://cdn.discordapp.com/a1.png"}'),
  ('22222222-2222-2222-2222-222222222222', 'cedho@example.com',
   '{"provider_id":"100000000000000002","full_name":"Cedho Nalen","avatar_url":"https://cdn.discordapp.com/a2.png"}'),
  ('33333333-3333-3333-3333-333333333333', 'tataru@example.com',
...
select discord_id, display_name, role from public.members order by discord_id;
...
select test.as_role('authenticated', '11111111-1111-1111-1111-111111111111', $q$
  with ins as (
    insert into public.availability (member_id, slot)
...
select test.as_role('authenticated', '22222222-2222-2222-2222-222222222222', $q$
  with ins as (
    insert into public.characters (member_id, lodestone_id, character_name, world)
...
left join public.availability a on a.member_id = m.id
...
select 'read availability rows' as attempt,
       test.as_role('anon', null,
         $q$ select count(*)::text from public.availability $q$) as result
union all select 'read members',
       test.as_role('anon', null,
         $q$ select count(*)::text from public.members $q$)
union all select 'read characters',
       test.as_role('anon', null,
         $q$ select count(*)::text from public.characters $q$)
union all select 'call is_leader()',
       test.as_role('anon', null,
         $q$ select public.is_leader()::text $q$)
union all select 'call availability_heatmap()  <- the one allowed path',
       test.as_role('anon', null,
         $q$ select string_agg(slot || ':' || available, '  ' order by slot)
             from public.availability_heatmap() $q$)
union all select 'call availability_stats()',
       test.as_role('anon', null,
         $q$ select 'members=' || members || ' respondents=' || respondents
             from public.availability_stats() $q$);

\echo ''
\echo '--- what the heatmap function actually returns (its whole result type) ---'
select * from public.availability_heatmap();
...
select 'SELECT * FROM availability (unfiltered)' as attempt,
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ select coalesce(string_agg(distinct m.display_name, ', '), '(no rows)')
             from public.availability a join public.members m on m.id = a.member_id $q$) as "whose rows come back"
union all select 'own slot count',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ select count(*)::text from public.availability $q$)
union all select 'SELECT * FROM members (unfiltered)',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ select string_agg(display_name, ', ' order by display_name) from public.members $q$)
union all select 'read member B''s rows by explicit id',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ select count(*)::text from public.availability
             where member_id = '33333333-3333-3333-3333-333333333333' $q$)
union all select 'INSERT a slot for member B',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ with ins as (insert into public.availability (member_id, slot)
...
union all select 'DELETE member B''s rows',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ with del as (delete from public.availability
...
union all select 'self-promote to leader',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ with up as (update public.members set role = 'leader'
...
union all select 'demote the actual leader',
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ with up as (update public.members set role = 'member'
...
select 'SELECT * FROM availability (unfiltered)' as attempt,
       test.as_role('authenticated', '11111111-1111-1111-1111-111111111111',
         $q$ select string_agg(distinct m.display_name, ', ')
             from public.availability a join public.members m on m.id = a.member_id $q$) as result
...
union all select 'named availability for slot 42',
       test.as_role('authenticated', '11111111-1111-1111-1111-111111111111',
         $q$ select string_agg(m.display_name, ', ' order by m.display_name)
             from public.availability a join public.members m on m.id = a.member_id
             where a.slot = 42 $q$)
union all select 'correct a bad character claim (reassign to member B)',
       test.as_role('authenticated', '11111111-1111-1111-1111-111111111111',
         $q$ with up as (update public.characters
...
select 'member A characters' as attempt,
       test.as_role('authenticated', '22222222-2222-2222-2222-222222222222',
         $q$ select coalesce(string_agg(character_name, ', '), '(no rows)')
             from public.characters $q$) as result
...
select test.as_role('anon', null,
  $q$ select string_agg(slot || ':' || available, '  ' order by slot)
      from public.availability_heatmap() $q$) as "anon heatmap";
